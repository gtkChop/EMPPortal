...
from emappext.leaves_mgmt.logic import leaves as leaves_api
log = logging.getLogger(__name__)


class EMAppLeaveManagementRegister(BaseAppCorePluginInterface, BaseAppRoutePluginInterface):
    """
    ...
    def app_api_actions(self, api):
        ...
        """
        api.register(
            action_name="leave_create",
            action_func=leaves_api.leave_create,
            method='POST'
        )
        api.register(
            action_name="leave_update",
            action_func=leaves_api.leave_update,
            method='POST'
        )
        api.register(
            action_name="leaves_show",
            action_func=leaves_api.leaves_show,
            method='GET'
        )
        return None
    ...
    def app_schema(self, schema):
        ...
        schema_dir = "emappext.leave_mgmt.schema"

        _core_schemas = {
            "leaves_schema": "leaves_schema.json"
        }
